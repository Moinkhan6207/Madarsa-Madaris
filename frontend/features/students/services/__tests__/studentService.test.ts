...
describe('studentService', () => {
  ...
  describe('list', () => {
    it('calls GET /tenant/students with params', async () => {
      (apiClient.get as jest.Mock).mockResolvedValue({ data: { students: [], meta: { total: 0, page: 1, limit: 20, totalPages: 0 } } });
  ...
  describe('create', () => {
    it('calls POST /tenant/students with data', async () => {
      const payload = { firstName: 'Ali', lastName: 'Khan', gender: 'MALE' as const, dateOfBirth: '2000-01-01', status: 'ACTIVE' as const };
  ...
  describe('changeStatus', () => {
    it('calls PATCH /tenant/students/:id/status with data', async () => {
      const payload = { status: 'INACTIVE' as const, reason: 'Graduated' };
  ...
  describe('addGuardian', () => {
    it('calls POST /tenant/students/:studentId/guardians', async () => {
      const payload = { relation: 'FATHER' as const, name: 'Khan', phone: '1234567890' };
      (apiClient.post as jest.Mock).mockResolvedValue({ data: { guardian: { id: 'g1', ...payload } } });
  ...
  describe('updateGuardian', () => {
    it('calls PUT /tenant/guardians/:guardianId', async () => {
      const payload = { name: 'Khan Updated' };
      (apiClient.put as jest.Mock).mockResolvedValue({ data: { guardian: { id: 'g1', ...payload } } });
  ...
  describe('mapSponsor', () => {
    it('calls POST /tenant/students/:studentId/sponsors', async () => {
      ...
      (apiClient.post as jest.Mock).mockResolvedValue({ data: { mapping: { id: 'm1', ...payload } } });
  ...
  describe('getHistory', () => {
    it('calls GET /tenant/students/:studentId/history', async () => {
      (apiClient.get as jest.Mock).mockResolvedValue({ data: { history: [] } });
