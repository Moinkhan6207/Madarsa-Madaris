...
describe('studentService', () => {
  ...
  describe('list', () => {
    it('calls GET /tenant/students with params', async () => {
      (api.get as jest.Mock).mockResolvedValue({ data: { data: [], pagination: { total: 0, page: 1, limit: 20, totalPages: 0 } } });
  ...
  describe('getById', () => {
    it('calls GET /tenant/students/:id', async () => {
      (api.get as jest.Mock).mockResolvedValue({ data: { data: { id: '1' } } });
  ...
  describe('create', () => {
    it('calls POST /tenant/students with data', async () => {
      (api.post as jest.Mock).mockResolvedValue({ data: { data: { id: '1' } } });
  ...
  describe('update', () => {
    it('calls PUT /tenant/students/:id with data', async () => {
      (api.put as jest.Mock).mockResolvedValue({ data: { data: { id: '1' } } });
  ...
  describe('changeStatus', () => {
    it('calls PATCH /tenant/students/:id/status with data', async () => {
      (api.patch as jest.Mock).mockResolvedValue({ data: { data: { id: '1' } } });
      await studentService.changeStatus('1', { status: 'ACTIVE', notes: 'Test' });
      expect(api.patch).toHaveBeenCalledWith('/tenant/students/1/status', { status: 'ACTIVE', notes: 'Test' });
    });
  });

  describe('addGuardian', () => {
    it('calls POST /tenant/students/:studentId/guardians', async () => {
      (api.post as jest.Mock).mockResolvedValue({ data: { data: { id: 'g1' } } });
  ...
  describe('updateGuardian', () => {
    it('calls PUT /tenant/guardians/:guardianId', async () => {
      (api.put as jest.Mock).mockResolvedValue({ data: { data: { id: 'g1' } } });
  ...
  describe('mapSponsor', () => {
    it('calls POST /tenant/students/:studentId/sponsors', async () => {
      (api.post as jest.Mock).mockResolvedValue({ data: { data: { id: 'm1' } } });
